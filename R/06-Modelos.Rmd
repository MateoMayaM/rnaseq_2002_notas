...
colnames(mat)
```
$$\beta$$1 es el cambio promedio en Y por cada incremento de una unidad en la variable x. 
Por cada incremento de 1 en el eje x aumenta el valor de Y. 
...
Cuando hacemos un modelo de expresión diferencial, si tenemos mil genes, vamos a hacer mil regresiones lineales. Como esto resultaría en muchas tablitas, únicamente extraeremos la información de un solo coeficiente (por ejemplo ```log Height``` en este caso). 
...
Nos da una interfaz gráfica que nos ayuda a entender de mejor manera los modelos de regresión lineal.  
...
Podemos restar "cuadrantes" para quedarnos solo con los coeficientes que necesitemos para dar una interpretación. 
